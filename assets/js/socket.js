...
channel.on("geAllWallets", function (payload) { // listen to the 'shout' event
  ...
  viewBlockchainDiv.style.display = "none"
  ...
  if (payload.wallets.length > 0) {
      ...
      innerDiv.innerHTML = '<h3 style="color: white;padding-left: 25px"> Successfully created 100 wallets. Click ViewWallets to view.</h3>'
...
channel.on("viewTransactions", function (payload) { 
  ...
  if (payload.confirmedTransactions) {
    walletNames = [];
    $("#transactionTables").display = "block"
    ...
    for (let index = 0; index < payload.confirmedTransactions.length; index++) {
      ...
      var sec = a.getSeconds();
      var time = date + ' ' + month + ' ' + year + ' ' + hour + ':' + min + ':' + sec ;
      transaction.noOfVerifications = transaction.noOfVerifications > 100? 100: transaction.noOfVerifications;
      $("#transactionTable tbody").prepend(
        ...
        <tr>
          ...
          <td>`+transaction.noOfVerifications+`%</td>
        </tr>
        `
      );
      ...
      $("#transactionTables").display = "none"
    }
      ...
      for (let index = 0; index < payload.unconfirmedTransactions.length; index++) {
        ...
        $("#unconfirmedTransactionTable tbody").prepend(
          ...
          <tr>
            ...
            <td>`+time+`</td>
            <td>`+transaction.totalAmount+`</td>
            <td>`+Math.floor(transaction.noOfVerifications * 0.5)+` %</td>
          </tr>
          `
        );
...
document.getElementById("generateTransactionButton").addEventListener("click", function(){
  ...
  sign.update(fromWallet.value + toWallet.value + "takemymoney");

  channel.push('generateTransaction', { // send the message to the server on "shout" channel
    fromWallet: fromWallet.value, toWallet: toWallet.value, amount: amount.value, sign:sign
    });
